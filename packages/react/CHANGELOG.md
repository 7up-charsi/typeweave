# @typeweave/react

## 4.2.21

### Patch Changes

- [`cf62558`](https://github.com/7up-charsi/typeweave/commit/cf6255881aaa0762f441ae3cbfa5b012a0ef2e4d) Thanks [@7up-charsi](https://github.com/7up-charsi)! - chore: fix 2

- [`3b323bc`](https://github.com/7up-charsi/typeweave/commit/3b323bc0913c2cff7880aec582b250dd57618cf9) Thanks [@7up-charsi](https://github.com/7up-charsi)! - chore: fix 3

- Updated dependencies [[`cf62558`](https://github.com/7up-charsi/typeweave/commit/cf6255881aaa0762f441ae3cbfa5b012a0ef2e4d), [`3b323bc`](https://github.com/7up-charsi/typeweave/commit/3b323bc0913c2cff7880aec582b250dd57618cf9)]:
  - @typeweave/react-utils@1.2.5

## 4.2.21

### Patch Changes

- [`990e134`](https://github.com/7up-charsi/typeweave/commit/990e13434221d8da4d50c0eef44511832722b1ae) Thanks [@7up-charsi](https://github.com/7up-charsi)! - chore: patch

- [`74efbdb`](https://github.com/7up-charsi/typeweave/commit/74efbdb349dd185c3bc62e9b58460353d7768ecb) Thanks [@7up-charsi](https://github.com/7up-charsi)! - chore: fix

- Updated dependencies [[`990e134`](https://github.com/7up-charsi/typeweave/commit/990e13434221d8da4d50c0eef44511832722b1ae), [`74efbdb`](https://github.com/7up-charsi/typeweave/commit/74efbdb349dd185c3bc62e9b58460353d7768ecb)]:
  - @typeweave/react-utils@1.2.5

## 4.2.22

### Patch Changes

- [`da09674`](https://github.com/7up-charsi/typeweave/commit/da09674526ad44738476cb4e4e3cfb22201b4571) Thanks [@7up-charsi](https://github.com/7up-charsi)! - chore: fix

- Updated dependencies [[`da09674`](https://github.com/7up-charsi/typeweave/commit/da09674526ad44738476cb4e4e3cfb22201b4571)]:
  - @typeweave/react-utils@1.2.6

## 4.2.21

### Patch Changes

- [#3](https://github.com/7up-charsi/typeweave/pull/3) [`842c1f8`](https://github.com/7up-charsi/typeweave/commit/842c1f8f9ce1cf1d0a98dfb2e4d9c7ee7c14ccc4) Thanks [@7up-charsi](https://github.com/7up-charsi)! - chore: eslint errors/warnings fix

- Updated dependencies [[`842c1f8`](https://github.com/7up-charsi/typeweave/commit/842c1f8f9ce1cf1d0a98dfb2e4d9c7ee7c14ccc4)]:
  - @typeweave/react-utils@1.2.5

## 4.2.20

### Patch Changes

- refactor: all packages
- Updated dependencies
  - @typeweave/react-utils@1.2.4

## 4.2.19

### Patch Changes

- fix(React): alert-dialog descritpion style

## 4.2.18

### Patch Changes

- fix(React): alert-dialog descritpion style

## 4.2.17

### Patch Changes

- fix(React): document is not defined

## 4.2.16

### Patch Changes

- fix: all known issues
- Updated dependencies
  - @typeweave/react-utils@1.2.3

## 4.2.15

### Patch Changes

- fix: use-scroll-lock more than one call with same element

## 4.2.14

### Patch Changes

- chore: switch back to tsc
- Updated dependencies
  - @typeweave/react-utils@1.2.2

## 4.2.13

### Patch Changes

- fix: bugs
  combobox: listbox opens even input does not get focus on touch devices
  combobox: when input has keyboard focus, page scrolls on arrow down
  combobox: readOnly does not work
  NumberInput: prop shouldFoucsInput
  Skeleton: no animation
- Updated dependencies
  - @typeweave/react-utils@1.2.1

## 4.2.12

### Patch Changes

- fix(React): Combobox getOptionLabel

## 4.2.11

### Patch Changes

- fix(React): input wrapper bg

## 4.2.10

### Patch Changes

- fix: prev release

## 4.2.9

### Patch Changes

- fix: border and foucs-ring color

## 4.2.8

### Patch Changes

- fix(React): switch disabled styles

## 4.2.7

### Patch Changes

- fix(React): checkbox a11y

## 4.2.6

### Patch Changes

- re-release

## 4.2.5

### Patch Changes

- feat: error state in checkbox

## 4.2.4

### Patch Changes

- fix(React): input styles when multine

## 4.2.3

### Patch Changes

- Updated dependencies
  - @typeweave/react-utils@1.2.0

## 4.2.2

### Patch Changes

- fix(React): input focus-visible ring

## 4.2.1

### Patch Changes

- fix: remove back forth route progress

## 4.2.0

### Minor Changes

- feat: use route progress

## 4.1.9

### Patch Changes

- fix(React): use scroll event dirY and dirX

## 4.1.8

### Patch Changes

- fix(React): change default dirY dirX at initial

## 4.1.7

### Patch Changes

- fix(React): use scroll event

## 4.1.6

### Patch Changes

- fix(React): use scroll hook default scrollable

## 4.1.5

### Patch Changes

- fix(React): use scroll hook

## 4.1.4

### Patch Changes

- feat(React): use scroll more values

## 4.1.3

### Patch Changes

- fix: remove extra addtion of clientHeight

## 4.1.2

### Patch Changes

- feat(React): useScroll total scroll

## 4.1.1

### Patch Changes

- feat(React): use scroll hook

## 4.1.0

### Minor Changes

- feat(React): use-swipeable

## 4.0.13

### Patch Changes

- fix(React): tooltip does not open on disabled chagne

## 4.0.12

### Patch Changes

- fix(React): tooltip does not hide when disabled after open

## 4.0.11

### Patch Changes

- fix(React): TooltipTrigger must be used inside TooltipRoot

## 4.0.10

### Patch Changes

- fix(React): tooltip can be disabled

## 4.0.9

### Patch Changes

- fix(React): drawer content aria role

## 4.0.8

### Patch Changes

- fix(React): endConent prop in combobox

## 4.0.7

### Patch Changes

- feat(React): add selected/removed option in onChange combobox

## 4.0.6

### Patch Changes

- fix(React): badge max width

## 4.0.5

### Patch Changes

- fix(React): button pointer events none

## 4.0.4

### Patch Changes

- fix(React): remove trigger toggle menu/popover

## 4.0.3

### Patch Changes

- fix: prev release

## 4.0.2

### Patch Changes

- refactor(React): add size-\*

## 4.0.1

### Patch Changes

- fix(React): add missing useClient directive

## 4.0.0

### Major Changes

- 65435a5: feat: strict types check with tsc and only esm

### Patch Changes

- dcdbb89: fix: exports in package.json
- Updated dependencies [65435a5]
- Updated dependencies [dcdbb89]
  - @typeweave/react-utils@1.1.4

## 4.0.0-next.1

### Patch Changes

- fix: exports in package.json
- Updated dependencies
  - @typeweave/react-utils@1.1.4-next.1

## 4.0.0-next.0

### Major Changes

- feat: strict types check with tsc and only esm

### Patch Changes

- Updated dependencies
  - @typeweave/react-utils@1.1.4-next.0

## 3.1.0

### Patch Changes

- feat(React): scroll aware app-bar

## 3.0.24

### Patch Changes

- fix(React): input styles when errored

## 3.0.23

### Patch Changes

- fix(React): data attributes

## 3.0.22

### Patch Changes

- fix(React): PointerEvents accepts ref

## 3.0.21

### Patch Changes

- fix(React): combobox controlled value
  feat(Plugin): allow user to customize internal light/dark theme

## 3.0.20

### Patch Changes

- fix(React): combobox and alert classNames

## 3.0.19

### Patch Changes

- fix(React): drawer

## 3.0.18

### Patch Changes

- feat(React): drawer focus trap

## 3.0.17

### Patch Changes

- fix(React): change overlay default color in dark mode

## 3.0.16

### Patch Changes

- fix(React): useScrollLock issue
  feat(React): useIsMounted hook

## 3.0.15

### Patch Changes

- fix(React): scrollLock hook locks scroll when closed and keepMounted

## 3.0.14

### Patch Changes

- fix(React): className doe not work in toogle button

## 3.0.13

### Patch Changes

- fix(React): input broken associantion btw input and helpertext

## 3.0.12

### Patch Changes

- fix(React): sr does not announce input error message

## 3.0.11

### Patch Changes

- fix(React): button does not submit form'

## 3.0.10

### Patch Changes

- fix(React): combobox disableClearable

## 3.0.9

### Patch Changes

- fix(React): combobox clear button and onChange reason type

## 3.0.8

### Patch Changes

- fix(React): combobox and input styles

## 3.0.7

### Patch Changes

- fix(React): radio, switch and checkbox styles'

## 3.0.6

### Patch Changes

- fix(React): radio, switch and checkbox styles

## 3.0.5

### Patch Changes

- fix(React): avatar missing ref

## 3.0.4

### Patch Changes

- fix(React): combobox console waring and props type

## 3.0.3

### Patch Changes

- fix(React): inputWrapper height when multiline

## 3.0.2

### Patch Changes

- fix(React): combobox open indicator submits form

## 3.0.1

### Patch Changes

- refactor(React): rename styles files
  fix(Plugin): types

## 3.0.0

### Major Changes

- feat: breaking changes
