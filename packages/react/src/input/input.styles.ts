...
export const inputStyles = tv({
  slots: {
    ...
    inputWrapper:
      'relative flex h-10 w-full cursor-text items-center gap-x-2 gap-y-1 overflow-hidden rounded border border-muted-7 bg-transparent px-3 dynamic-icon has-[input:focus-visible]:border-transparent has-[input:focus-visible]:ring-2 has-[input:focus-visible]:ring-focus hover:border-muted-8 has-[input:focus-visible]:hover:border-transparent data-[multiline=true]:h-auto data-[error=true]:border-danger-7 data-[multiline=true]:p-0 data-[error=true]:has-[input:focus-visible]:border-transparent data-[error=true]:has-[input:focus-visible]:ring-danger-8 data-[error=true]:hover:border-danger-8 data-[error=true]:has-[input:focus-visible]:hover:border-transparent',
    input:
